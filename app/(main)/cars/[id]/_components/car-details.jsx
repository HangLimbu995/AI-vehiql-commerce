...
const CarDetails = ({ car, testDriveInfo }) => {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      navigator
        .share({
          title: `${car.year} ${car.year} ${car.model}`,
          text: `Check out this ${car.year} ${car.make} ${car.model} on Vehiql`,
          url: window.location.href,
        })
        .catch((error) => {
          console.error("Error shring", error);
          copyToClipboard();
        });
    } else {
      copyToClipboard();
  ...
  const calculateMonthlyPayment = (
    ...
    const financedAmount = Math.max(principal - downPayment, 0);
    if (totalMonths <= 0) return 0;
    const monthlyRate = annualInterestRate / 12;
    if (monthlyRate === 0) return financedAmount / totalMonths;
    const growthFactor = Math.pow(1 + monthlyRate, totalMonths);
    return (financedAmount * monthlyRate * growthFactor) / (growthFactor - 1);
  };

  const monthlyPayment = calculateMonthlyPayment(car.price, 0.045, 60, 0);
  return (
    <div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {car.images.map((image, index) => {
                return (
                  <div
                    key={index}
                    className={`relative cursor-pointer rounded-md h-20 w-24 flex-shrink-0 transition overflow-hidden ${
            ...
            <Dialog>
              <DialogTrigger className="w-full text-start">
                <Card>
                  <CardContent>
                    ...
                    <div className="text-xs text-gray-500 mt-l">
              ...
              <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
                ...
                <EMICalculator
                  price={car.price}
                  image={
                    car.images && car.images.length ? car.images[0] : undefined
                  }
